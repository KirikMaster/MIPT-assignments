...
class SimplifiedBaggingRegressor:
    ...
    def predict(self, data):
        ...
        '''
        return np.mean(np.array([self.models_list[bag].predict(data) for bag in range(self.num_bags)]), axis=0)
    ...
    def _get_oob_predictions_from_every_model(self):
        ...
        for i in range(len(self.data)):
            ...
            for bag in range(len(self.indices_list)):
                bag_indices = self.indices_list[bag]
                if i not in bag_indices:
                    bag_model = self.models_list[bag]
                    prediction = bag_model.predict(obj)
    ...
    def _get_averaged_oob_predictions(self):
        ...
        for i in range(len(self.data)):
            if self.list_of_predictions_lists[i] != []:
    ...
    def OOB_score(self):
        ...
        for i in range(len(self.oob_predictions)):
            if self.oob_predictions[i] != None:
                MSE += (self.oob_predictions[i] - self.target[i])**2
