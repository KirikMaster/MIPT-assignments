...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    norms_squared = torch.sum(torch.square(x_1), 1, keepdim=True) - 2*x_1 @ x_2.T + torch.sum(torch.square(x_2.T), 0)
    distances = torch.exp(-1 / (2*sigma**2) * norms_squared)
    
    #norms_squared = np.sum(np.square(x_1), axis = 1).reshape((x_1.shape[0], 1)) - 2*x_1 @ x_2.T + np.sum(np.square(x_2.T), axis = 0)
    #distances = np.exp(-1/ (2*sigma**2) * norms_squared)
...
def hinge_loss(scores, labels):
    ...
    return 1/len(scores) * torch.max(torch.Tensor([0]), 1 - torch.dot(scores, labels))
    #return torch.mean(torch.max(torch.zeros(len(labels)), torch.ones(len(labels)) - scores*labels))
