...
global task;
task = 'inertial'; % orbital inertial
...
sig_J = 1e-3;
sig_Jorient = 1e-3;
G = EulerMatrix(randn2(0, sig_Jorient), randn2(0, sig_Jorient), randn2(0, sig_Jorient));
J_e = G.' * randn2(J, sig_J) * G;
...
D_init = quat2dcm(Q_init); % ИСК -> ССК
...
for i = 1:N - 1
    ...
    A0c = A0(:, :, i);
    wc = wabs(:, i);
    B = Magnetic_field(rc, params);
    [D_tr, w_tr] = TRIAD(rc, Dc, wc, params);
    data.M_grav = 3*params.mu / norm(rc)^5*cross(Dc*rc, J*Dc*rc); % ССК
    data.M_magn = cross(params.mm, B); % ССК
    data.M_ctrl = Lyapunov(rc, A0c, D_tr, w_tr, J_e, data, params);
    M_ctrl(:, i) = data.M_ctrl;
...
plot(t, wabs, Visible="on")
plot(t, w, Visible="off")
