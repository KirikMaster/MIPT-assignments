...
Q_init = quatnormalize([cos(incl_init/2), sin(incl_init/2)*rotvect.']); % Из ИСК в ССК
D_init = quat2dcm(Q_init); % Из ИСК в ССК
...
%% Integration
for i = 1:N - 1
    ret = integrator(@GravMoment, [r(:, i)', v(:, i)', Q(i, :), wabs(:,i)'], t(i), dt, params);
    r(:, i+1) = ret(1:3)'; % ИСК
    v(:, i+1) = ret(4:6)'; % ИСК
...
plot(t, centralize(Jacobi2), Visible="on");
legend('Jacobi', 'Jacobi2')
...
quiver3(r(1,start:k:end)/params.R, r(2,start:k:end)/params.R, r(3,start:k:end)/params.R, A0(1,start:k:end,3), A0(2,start:k:end,3), A0(3,start:k:end,3), scOF, LineWidth=LWOF, Color='black', Visible='on')
% ССК
